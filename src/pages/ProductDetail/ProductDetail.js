     ...
     return  filteredOptions.map((item, key) => {
        ...
        <Fragment>
          <Button
            keys={key}
            content={item.name}
            color={selectedOption.includes(item._id) ? {
              color: "white",
              backgroundColor: "#2C3E50"
            }: null}
            handleClick={() => this.handleAttrOptionClick(filteredOptions,selectedOption, productAttrId, item._id)}
          />
      </Fragment>
      )})
  }
  ...
  render() {
    const { isLoading,
         productDetailById : {
          ...
          }, productDetailById, selectedProductAttr }  = this.props;
      ...
      return (
        <div className="container product-detail-container">
           {!!Object.keys(selectedProductAttr).length && <div className="row">
            <div className="col-12 col-md-6 image-container mx-auto align-center text-center">
             ...
             <div className="col-12 col-md-6 mt-2 mb-5">
               <h6 className="font-weight-bold text-uppercase">{selectedProductAttr.name}</h6>
              <p className="description">
                  {primary_product.desc}
               </p> 
              <hr />
               <strong> Rs. {parseFloat(selectedProductAttr.sale_price).toFixed(2)}</strong>
              &nbsp;&nbsp;&nbsp; 
               
              <Fragment>
              <strike>Rs. {parseFloat(selectedProductAttr.mark_price).toFixed(2)}</strike> <br />
              <small className="sales-msg">
              You save Rs. { parseFloat(selectedProductAttr.mark_price - selectedProductAttr.sale_price).toFixed(2)} {selectedProductAttr.sale_msg}</small>
              <br /> 
               </Fragment>
...
const mapStateToProps = ({ product: { isLoading, productDetailById, selectedProductAttr } }) => ({
  isLoading,
  productDetailById,
  selectedProductAttr
});
